...
describe("parseArgs — --cwd flag", () => {
	...
	it("uses the system temporary directory for Windows home launches", async () => {
		...
		// A persisted `OMP_ALLOW_HOME` (the agent .env) short-circuits the auto-chdir
		// before the win32 candidate list, so this case pins its own precondition.
		const previousAllowHome = process.env.OMP_ALLOW_HOME;
		...
		try {
			...
			if (previousAllowHome !== undefined) process.env.OMP_ALLOW_HOME = previousAllowHome;
		}
	});
});
...
describe("applyStartupCwd — home auto-chdir", () => {
	it("keeps the home directory as the project when OMP_ALLOW_HOME is set", async () => {
		...
		try {
			...
			expect(utils.normalizePathForComparison(process.cwd())).toBe(utils.normalizePathForComparison(home));
		} finally {
			delete process.env.OMP_ALLOW_HOME;
		}
	});
	...
	it("still switches away from home when OMP_ALLOW_HOME is explicitly off", async () => {
		...
		try {
			...
			expect(utils.normalizePathForComparison(utils.getProjectDir())).not.toBe(
				utils.normalizePathForComparison(home),
			);
		} finally {
			delete process.env.OMP_ALLOW_HOME;
		}
