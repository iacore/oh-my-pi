...
import { cfgDisplayHideToolOutputDetails } from "@oh-my-pi/pi-coding-agent/modes/settings";
...
describe("InputController global editor actions", () => {
	...
	it("folds tool output details behind one display gesture", async () => {
		const context = await createContext();
		const controller = new context.InputController(context.ctx);

		controller.toggleDetailVisibility();

		expect(context.ctx.hideToolOutputDetails).toBe(true);
		expect(cfgDisplayHideToolOutputDetails.get(context.ctx.settings)).toBe(true);
		expect(context.ctx.chatContainer.setToolOutputDetailsHidden).toHaveBeenCalledWith(true);
	});
